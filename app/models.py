...
class Reviews(db.Model):
    ...
    user = db.relationship('User', foreign_keys=[
                           userId], backref='reviews')
    like = db.relationship('Likes', backref='reviews',
                           cascade='all, delete-orphan')
    stadium = db.relationship('Stadiums', foreign_keys=[
                              stadiumId], backref='reviews')

    def __repr__(self):
        return f"Review('{self.title}','{self.review}','{self.date}',\
    '{self.rating}')"
...
class Likes(db.Model):
    ...
    review = db.relationship('Reviews', foreign_keys=[
                             reviewId], backref='likes')
