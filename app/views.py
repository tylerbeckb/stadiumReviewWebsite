...
def index():
    ...
    mostLiked = db.session.query(Likes.reviewId, func.count(Likes.id)).\
        group_by(Likes.reviewId).order_by(func.count(Likes.id).desc()).\
        limit(3).all()
...
def login():
    ...
    if loginForm.validate_on_submit():
        user = User.query.filter_by(username=request.form['loginName']).\
            first()
        # Checks if username exists
        if user:
            # Checks if password matches
            if check_password_hash(user.password,
                                   request.form['loginPassword']):
                # Logs in user
                login_user(user)
...
def signup():
    ...
    if signUpForm.validate_on_submit():
        ...
        if exists is None:
            record = User(name=signName, username=signUsername,
                          password=generate_password_hash(signPassword))
            db.session.add(record)
            db.session.commit()
...
def review(name):
    ...
    if reviewForm.validate_on_submit():
        ...
        record = Reviews(title=title, review=text, date=dateObject,
                         rating=rating, stadiumId=stadId,
                         userId=current_user.id)
        db.session.add(record)
        db.session.commit()
        app.logger.info("%s added a review for %s",
                        current_user.username, stad.name)
        return redirect(url_for('index'))
...
def deleteReview(id):
    ...
    db.session.commit()
    app.logger.info("%s deleted a review of id: %d",
...
def vote():
    ...
    if data.get('voteType') == "not":
        ...
        db.session.add(record)
        db.session.commit()
        app.logger.info("%s liked a review of id: %d",
                        current_user.username, reviewId)
    else:
        Likes.query.filter_by(
            reviewId=reviewId, userId=current_user.id).delete()
        db.session.commit()
        app.logger.info("%s unliked a review of id: %d",
                        current_user.username, reviewId)
